...
class FetchGraph:
    ...
    def fetch_default_graph(self, type, g=None):
        if g is None:
            g = Graph()
        doc_id = 0
        g = build_subgraph(g, self.qb.build_query_attributes(type), self.conection_string)
        return g
    ...
    @dispatch(int, int, Graph)
    def fetch_graph(self, start, stop, g):
        #g = Graph()
        doc_id = 0
        for i in range(start, stop):
            g = build_subgraph(g, self.qb.build_query_by_sentence_id(doc_id, i), self.conection_string)
    ...
    @dispatch(list, Graph)
    def fetch_graph(self, ids_list, g):
        #g = Graph()
        doc_id = 0
        for i in ids_list:
            g = build_subgraph(g, self.qb.build_query_by_sentence_id(doc_id, i), self.conection_string)
