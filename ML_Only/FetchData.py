...
def fetch_graph(edges, nodes, indexes, ids_to_fetch, config_data, test):
    ...
    is_something = config_data['connection'][0]['is_something']
    ...
    for i in tqdm(range(len(ids_to_fetch)), desc="Loading Dataset"):
        ...
        if is_something:
            graph = change_targets_ner(graph, base_uri)
                ...
                for layer in config_data['nodes']:
                    ...
                    for s, p, o in graph.triples((None, RDF.type, URIRef(layer['uri']))):
                        ...
                        for s2, p2, o2 in graph.triples((URIRef(s), None, None)):
                            ...
                            if uri_type in layer['attributes'] or uri_type in config_data['target'][0]['name']:
                                node_features[uri_type] = o2.__str__()
                            elif uri_type not in config_data['exclude'][0]['name']:
                                if uri_type not in edges:
                                    edges[uri_type] = []
                                edges[uri_type].append([s2.__str__(), o2.__str__()])
...
def change_targets_ner(g, base_uri):
    tags_uri = URIRef(base_uri + "OIE_tags")
    something_uri = URIRef(base_uri + "isSomething")
    for s, p, o in g.triples((None, tags_uri, None)):
        rel_type = o.__str__()
        if rel_type == "No" or rel_type == "O" or rel_type == "0" or rel_type == "O\n":
            g.add((s, URIRef(something_uri), Literal("No")))
        else:
            g.add((s, URIRef(something_uri), Literal("Yes")))
    return g
...
def build_graph(nodes, edges, mapped_ids, config_data, graph_data , test = False, test_targets=None, embedding = True):
    for layer in config_data['nodes']:
        ...
        if layer['name'] == 'word':
            ...
            if config_data['connection'][0]['edge_connections']:
                for k, v in edges.items():
                    if k not in layer['edges'] and k not in config_data['exclude'][0]['word_name']:
                        layer['edges'].append(k)
                        layer['edges_source'].append('word')
                        layer['edges_target'].append('word')
        else:
            nodes_appropriate = nodes_df[column_list]
            nodes_tensor = torch.from_numpy(nodes_appropriate.values).to(torch.float)
