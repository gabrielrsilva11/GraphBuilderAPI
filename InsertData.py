...
class CreateGraph:
    ...
    def insert_script(self, lines, sentence_id):
        ...
        for index, row in conll.iterrows():
            ...
            if row['id'] == 1:
                ...
                self.insert_data(self.c_text_uri, self.o_contains_sentence, sentenceid_uri, self.sparql)
                if sentence_id != 1:
                    self.insert_data(self.c_sentence_uri + "_" + str(sentence_id - 1), self.o_nextsentence_uri, sentenceid_uri, self.sparql)
            else:
                word_id = row['id']
            ...
            if row['head'] == 0:
                ...
                self.insert_data(wordid_uri, self.o_head_uri, self.d_word_uri + "_" + str(sentence_id) + "_" + str(row['head']), self.sparql)
                self.insert_data(self.d_word_uri + "_" + str(sentence_id) + "_" + str(row['head']), self.o_depgraph_uri, wordid_uri, self.sparql)
        self.insert_data(sentenceid_uri, self.d_sentence_text, Literal(' '.join(sentence)), self.sparql)
        self.insert_data(self.c_text_uri, self.o_contains_sentence, sentenceid_uri, self.sparql)
        return sentence_id

    def insert_wikimapper_data(self, sentence_id, id):
        """
        ...
        query = self.queries.build_insert_wikimapper_query(sentence_uri, self.o_mapper_uri, Literal(id))
    ...
    def create_graph(self):
        ...
        :return:
        """
        num_lines = sum(1 for line in open(self.file_name, 'r'))
        sentence_id = 0
        i = 0
        lines = ''
...
# graph = CreateGraph(doc="data/wikiner_subset.txt", relations_uri=relations_uri, connection_string=connection, language='pt')
# graph.create_graph()
