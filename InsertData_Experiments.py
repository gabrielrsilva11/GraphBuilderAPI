...
class CreateGraph:
    ...
    def insert_memory_relationship_data(self, g):
        ...
        g.add((URIRef(self.c_attributes_uri), RDF.type, OWL.Class))
        ...
        g.add((URIRef(self.o_pos_uri), RDFS.subClassOf, URIRef(self.c_attributes_uri)))
        g.add((URIRef(self.o_poscoarse_uri), RDFS.subClassOf, URIRef(self.c_attributes_uri)))
        g.add((URIRef(self.o_edge_uri), RDFS.subClassOf, URIRef(self.c_attributes_uri)))
        g.add((URIRef(self.o_feats_uri), RDFS.subClassOf, URIRef(self.c_attributes_uri)))
            ...
            for feat in split_feats:
                ...
                if feat[0] in self.d_feats_list:
                    if feat[1] in self.feats_specific_list:
                        if self.in_memory:
                            g.add((wordid_uri, RDF.type, URIRef(self.main_uri+feat[1].lower())))
                        else:
                        ...
                        if self.in_memory:
                            g.add((URIRef(self.main_uri + feat[1].lower()), RDFS.subClassOf, URIRef(self.main_uri + "feat_" + feat[0].lower())))
                            g.add((wordid_uri, RDF.type, URIRef(self.main_uri+feat[1].lower())))
                        else:
                            self.insert_data(self.main_uri + feat[1].lower(), RDFS.subClassOf, self.main_uri + "feat_" + feat[0].lower(), self.sparql)
                            self.insert_data(wordid_uri, self.main_uri+"feat_"+feat[0].lower(), Literal(feat[1]), self.sparql)
                else:
                    if self.in_memory:
                        self.d_feats_list.append(feat[0])
                        self.feats_specific_list.append(feat[1])
                        g.add((URIRef(self.main_uri+"feat_"+feat[0].lower()), RDFS.subClassOf, URIRef(self.o_feats_uri)))
                        g.add((URIRef(self.main_uri + feat[1].lower()), RDFS.subClassOf, URIRef(self.main_uri+"feat_"+feat[0].lower())))
                        g.add((wordid_uri, RDF.type, URIRef(self.main_uri +feat[1].lower())))
                    else:
                        self.d_feats_list.append(feat[0])
    ...
    def process_conll_as_objects(self, prop_type, word, uri, to_add, graph = None):
        if prop_type == "edge":
            ...
            if to_add not in self.edges_list:
                ...
                if self.in_memory:
                    graph.add((URIRef(to_add_uri), RDFS.subClassOf, URIRef(uri)))
                else:
