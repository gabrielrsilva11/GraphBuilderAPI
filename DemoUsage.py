...
main_uri = "http://ieeta.pt/ontoud#"
qb = QueryBuilder(main_uri)
g = Graph()

query = qb.build_query_by_and_sentence_list(['Portugal', 'Algarve', 'Europa'])
print(query)
...
for s, p, o in g.triples((None, senttext_uri, None)):
    print(s, p, o)
    grafo = list_conll_subgraph(graph=g, root_node=s, transverse_by=depgraph_uri, order_by=id_uri)
    pprint.pprint(grafo)
    break
