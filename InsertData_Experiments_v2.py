...
class CreateGraph:
    ...
    def insert_memory_script(self, lines, sentence_id, doc_id, g):
        ...
        for index, row in conll.iterrows():
            ...
            #new objects
            # g.add((wordid_uri, URIRef(self.o_edge_uri), Literal(row['DEPREL'])))
            # g.add((wordid_uri, URIRef(self.o_pos_uri), Literal(row['UPOS'])))
            # g.add((wordid_uri, URIRef(self.o_poscoarse_uri), Literal(row['XPOS'])))
